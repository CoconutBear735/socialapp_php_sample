 ...
 * エンコードdocomo
 */
class Emo_Encode_docomo extends Emo_Encode_Core
{
	// {{{ properties
	 ...
	 */
	function Emo_Encode_docomo(&$context)
	{
	 ...
	 * @return	string	$this->_doxxx($str)	変換後	例）晴れ[[d-E63E]]曇り[[d-E63F]]
	 */
	function main($str)
	{
		if (!$this->_code) {
			$this->_code =& new Emo_Code_docomo;
		}
		// 実行
		if ($this->_code_type["set_docomo"] == "sjis") {
			return $this->_doSJIS($str);
		} else if ($this->_code_type["set_docomo"] == "utf-8") {
	...
	function _exec($str, $emoji)
	{
		if (preg_match_all("/(?:$emoji)/", $str, $matches)) {
			...
			foreach ($matches[0] as $key => $mcode) {
				$code = $this->_code->toKey($mcode, $this->_code_type['set_docomo']);
				$str = str_replace($matches[0][$key], $this->_left_delimiter . "d-" . $code . $this->_right_delimiter, $str);
			}
		}
		return $str;
	}
