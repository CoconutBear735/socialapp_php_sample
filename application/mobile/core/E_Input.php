...
class E_Input extends CI_Input {
	...
	*/
	...
	function apache_headers($index = NULL, $xss_clean = FALSE)
	{
		$outh_header = apache_request_headers();
		Log::ObjectDataOut($outh_header);
		
		// Check if a field has been provided
		if ($index === NULL AND ! empty($outh_header)) {
		
			$request = array();
			
			// Loop through the full _POST array and return it
			foreach (array_keys($outh_header) as $key) {
				$request[$key] = $this->_fetch_from_array($outh_header, $key, $xss_clean);
			}
			
			return $request;
		}
		
		return $this->_fetch_from_array($outh_header, $index, $xss_clean);
	}
}
